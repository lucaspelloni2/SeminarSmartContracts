...
The most famous application of blockchain is in online trades where it gets used for transfering a cryptovalue, bitcoins.\cite{blockchain8}
%TODO 
...
After publishing the contract, the hacker sent some Ether to Mallory and automatically invokes the Mallory Fallback\footnote{A Fallback is just a function without any name or parameter, which is called by default if some ether is sent to a contract\cite{SC10}. } (line 16, \textbf{Mallory}). \\
...
\\Now, let's have a closer look at the contract:
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.5]{Talk3/img/oad1}
\end{center}
...
\bibitem{A}\emph{Bits on blocks: Thoughts on blockchain technology}\url{https://bitsonblocks.net/2016/02/01/a-gentle-introduction-to-smart-contracts} (last accessed March 2017)
